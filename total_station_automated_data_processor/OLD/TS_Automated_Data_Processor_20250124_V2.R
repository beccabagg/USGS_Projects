...
server <- function(input, output) {
  ...
  output$download_xml <- downloadHandler(
    ...
    content = function(file) {
      ...
      formatted_date <- paste0(input$date, "T09:51:00-07:00")
      ...
      xml_add_child(site, "SiteIdentifier", organizationCode = "USGS") %>%
        xml_set_text(input$site_number)
      ...
      for (i in 1:nrow(summary_data)) {
        ...
        xml_add_child(object, "ObjectName") %>%
          xml_set_text(ifelse(is.na(summary_data$`Point name`[i]), "", as.character(summary_data$`Point name`[i])))
      ...
      cat('<?xml version="1.0" encoding="utf-8"?>', file = file, append = TRUE)
      cat('<?xml-stylesheet type="text/xsl" href="http://water.usgs.gov/XML/AQ/stylesheets/AQSVMobile_SS1.xsl"?>', file = file, append = TRUE)
      cat('<UsgsHydroML xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://water.usgs.gov/XML/AQ" schemaLocation="http://water.usgs.gov/XML/AQ http://water.usgs.gov/XML/AQ/schema/UsgsHydroML.xsd">', file = file, append = TRUE)
      
      # Save the rest of the XML to file
      write_xml(xml, file, append = TRUE)
    }
  )
}
