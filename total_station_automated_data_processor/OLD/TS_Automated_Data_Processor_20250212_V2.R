...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      textInput("end_time", "Enter End Time (HH:MM:SS)"),  # Added end time input
      actionButton("update_info", "Update Info"),
      hr(),
      
      textInput("base_object", "Enter Object Name to be Renamed as BASE in the output XML"),
...
server <- function(input, output) {
  ...
  output$download_xml <- downloadHandler(
    ...
    content = function(file) {
      ...
      end_datetime <- paste(input$date, input$end_time, sep = "T")
      end_datetime <- paste(end_datetime, "-07:00", sep = "")
      
      # Extract Total Error [sft] from the metadata
      total_error_value <- total_error()
      if (is.na(total_error_value)) {
        total_error_value <- "N/A"  # Handle case where total_error_value is NA
      } else {
        total_error_value <- as.character(total_error_value)  # Convert to character
      }
      ...
      xml_add_child(site_visit, "EndDateTime") %>%
        xml_set_text(end_datetime)
      xml_add_child(site_visit, "PartyText") %>%
        xml_set_text(party_name)
      ...
      xml_add_child(gaging_station_levels_circuit, "ClosureError") %>%
        xml_set_text(total_error_value)
      ...
      # Manually construct the XML header and root element
      ...
      xml_content <- sub('<?xml version="1.0" encoding="UTF-8"?>', '', xml_content)
      xml_content <- paste(xml_header, xml_content, sep = "")
      
      # Save XML to file
      write(xml_content, file)
    }
  )
}
