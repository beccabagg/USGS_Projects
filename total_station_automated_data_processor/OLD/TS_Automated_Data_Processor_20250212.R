...
server <- function(input, output) {
  ...
  observeEvent(input$update_info, {
    req(input$party_name, input$site_number, input$date, input$time)
    
    info_data <- data.frame(
      Variable = c("Party Name", "Site Number", "Date", "Time"),
      Value = c(input$party_name, input$site_number, input$date, input$time),
  ...
  output$download_xml <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Save XML to file with the specified first three lines
      xml_content <- as.character(xml)
      xml_content <- paste('<?xml version="1.0" encoding="utf-8"?>',
